	...
	if ($error != '') {
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"2762\",\"R4LDhtml\",\"$src\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"2762\",\"R4LDhtml\",\"$src\",'sub','N')";
		...
		if ($build_list_id != '') {
			...
			$send_to_arcamax = Arcamax($email,$build_list_id,'2762',$user_ip,'sub'); // sub or unsub
			...
			$insert_log = "INSERT IGNORE INTO arcamaxNewLog (dateTime,email,listid,subcampid,ipaddr,type,response)
						VALUES (NOW(),\"$email\",\"$build_list_id\",\"2762\",\"$user_ip\",\"sub\",\"$send_to_arcamax\")";
...
<table width="685px" height="400px" align="center" border="0" cellpadding="0" cellspacing="0" style="font-size:11px;font-family: verdana;background-image:url('http://pics.recipe4living.com/r4l_search_LP2.jpg');background-repeat:no-repeat;">
<tr valign="top">
...
<script type="text/javascript">
	var _gaq = _gaq || [];
<?php if ($host == 'r4l') { ?>
	_gaq.push(['_setAccount', 'UA-1200417-21']);
<?php } if ($host == 'fitfab') { ?>
	_gaq.push(['_setAccount', 'UA-10900002-7']);
<?php } ?>
