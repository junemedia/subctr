...
if ($_POST['submit'] == 'Enter to Win!') {
	...
	switch ($guid) {
		...
		case 'C74A2AF66E384FB68487DB27AC74A1B4':
			$subcampid = '3474';    //R4L Google Sweepstakes 0913
			break;
		case 'DBD36470CCCE4730BF397E9991E5C66E':
			$subcampid = '3513';   //Center Point Coreg - R4L Giveaway -0913 and  1013
			break;
		...
		case 'A921027365F24F7EB81E2864F107FEEA':
			$subcampid = '3568';   // Day Online Solutions - R4L Giveaway 1013
			break;
		case '99E4F124AFDB42588BDD53799984EF62':
			$subcampid = '3572';   // CenterPoint 1113
			break;
		case '690135A6F3F94D24A970A26B5FCE10EF':
			$subcampid = '3573';   // Day Online Solutions 1113
			break;
		case '0CF4A30C712D46738F8338AD926E3861':
			$subcampid = '3574';  //Shopathome 1113
			break;
		case '8E36A65647EF42E08FDC7FFD31A4C919':
			$subcampid = '3575';  //Sweepstakes Advantage 1113
			break;
		case '87598D3D3C144A89AFB79D9CF1AB5489':
			$subcampid = '3576';   //QuickRewards  F&F 1113
			break;
		case '4663AF6BE7E644FEA3DB7C26F0AAAC4D':
			$subcampid = '3590';   // ID213 R4L Co-Reg
			break;
		case '47FF88193A874923B08C618E7781AB4D':
			$subcampid = '3644';	// R4L Facebook Paid Search
			break;
		default:
			$subcampid = '4373';	// R4L Default Giveaway 0815
	}
	
	/*if ($fname == '') {
		$error = 'First name is required field.';
	} else {
		if (!ctype_alpha($fname)) {
			$error = 'First name must only contain alpha characters.';
		}
	}*/
	
	/*if ($lname == '') {
		$error = 'Last name is required field.';
	} else {
		if (!eregi("^[-A-Z[:space:]'\.]*$", $lname)) {
			$error = 'Please provide valid last name.';
		}
	}
	
	if (!in_array($age_group, array('Under 30','31-40','41-50','51-60','61-70','70+'))) {
	...
	*/
	...
	if ($error != '') {
		...
		$check_result = mysql_query("SELECT * FROM userData WHERE email = \"$email\" LIMIT 1");
		echo mysql_error();
		if (mysql_num_rows($check_result) == 0) {
			// do insert
			$insert = "INSERT INTO userData (email,fname) VALUES (\"$email\",\"$fname\")";
			$insert_result = mysql_query($insert);
			echo mysql_error();
		} else {
			// do update
			$update = "UPDATE userData SET 	fname= \"$fname\" 	WHERE email = \"$email\"";
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$subcampid\",\"R4LGiveaway\",\"$src\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"$subcampid\",\"R4LGiveaway\",\"$src\",'sub','N')";
		...
		$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=s&f=R4LGiveaway$src' width='0' height='0' border='0' />"."<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=s&f=R4LGiveaway$guid' width='0' height='0' border='0' />".$gtm_pixel;
		
        $offid = isset($_REQUEST['offid'])?$_REQUEST['offid']:'';
		$thankyou_page = "http://www.recipe4living.com/giveaway/thankyou.htm?e=$email&cp=$cake_pixel&offid=$offid&fname=$fname";
	...
	$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=d&f=R4LGiveaway$src' width='0' height='0' border='0' />"."<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=d&f=R4LGiveaway$guid' width='0' height='0' border='0' />";
}

?>
<html>
<head>
<title>Recipe4Living.com Giveaway</title>
...
<input type="hidden" name="offid" value="<?php if(isset($_REQUEST['offid'])){ echo $_REQUEST['offid'];}?>" />
<table border="0" cellpadding="0" cellspacing="5"  width="395px">
<tr><td colspan="2">&nbsp;</td></tr>
<?php echo $message; if ($signup_success) { exit; } ?>
            
			<tr><td colspan="2">Required <font style="color:red;">*</font></td></tr>
			<tr>
				<td>First Name</td>
				<td><font style="color:white;">*</font> <input type="text" name="fname" id="fname" value="<?php echo $fname; ?>"></td>
			</tr>
			<tr>
				<td>Email Address</td>
				<td><font style="color:red;">*</font> <input type="text" name="email" id="email" value="<?php echo $email; ?>"></td>
			</tr>
			<tr><td colspan="2">&nbsp;</td></tr>
