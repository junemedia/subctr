	...
	if ($error != '') {
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"2761\",\"R4lChristmasDhtml\",\"R4lChristmasDhtml\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"2761\",\"R4lChristmasDhtml\",\"R4lChristmasDhtml\",'sub','N')";
		...
		$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=s&f=R4lChristmasDhtml' width='0' height='0' border='0' />";
	}
} else {
	$attempt = false;
	$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=d&f=R4lChristmasDhtml' width='0' height='0' border='0' />";
}
...
<table width="610px" height="300px" align="center" border="0" cellpadding="0" cellspacing="0" style="font-size:11px;font-family: verdana;background-image:url('http://pics.recipe4living.com/email_cap/Christmas_Dhtml_email_cap.jpg');background-repeat:no-repeat;">
<tr valign="top">
...
<?php echo $message; if ($signup_success == true) { echo "<meta http-equiv='refresh' content='1;url=http://www.sayyourthoughts.com/dispatch2.asp?home=298-28961W&email=$email'>";exit; } ?>
<tr>
...
<tr>
	<td colspan="2" align="center">
		<font size="1"><a href="http://www.recipe4living.com/privacy" target="_blank">Privacy Policy</a></font>
	</td>
</tr>
</table>
</form>
