	...
	if ($error != '') {
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"3111\",\"FFDhtml\",\"$src\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"3111\",\"FFDhtml\",\"$src\",'sub','N')";
		...
		if ($build_list_id != '') {
			...
			$send_to_arcamax = Arcamax($email,$build_list_id,'3111',$user_ip,'sub'); // sub or unsub
			...
			$insert_log = "INSERT IGNORE INTO arcamaxNewLog (dateTime,email,listid,subcampid,ipaddr,type,response)
						VALUES (NOW(),\"$email\",\"$build_list_id\",\"3111\",\"$user_ip\",\"sub\",\"$send_to_arcamax\")";
		...
		$message = "<tr><td colspan='2' style='color:black;padding:20px;' align='center' valign='top'><h4>
					...
					</h4><script>setTimeout('redirect()', 5000);</script></td></tr>";
...
<script>
function redirect() {
    window.top.location.href="<?php echo $curr_page; ?>";
}
</script>
</head>
<body>
...
<input type="hidden" value="<?php echo $curr_page; ?>" name="curr_page" id="curr_page">
<table width="640px" height="380px" align="center" border="0" cellpadding="0" cellspacing="0" style="font-size:11px;font-family: verdana;background-image:url('http://pics.fitandfabliving.com/FF_shrimp_dhtml.jpg');background-repeat:no-repeat;">
<tr valign="top">
