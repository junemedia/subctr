...
if ($_POST['submit'] == 'Sign Me Up!') {
	...
	if ($error != '') {
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$subcampid\",\"WIMDHTML\",\"$src\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"$subcampid\",\"WIMDHTML\",\"$src\",'sub','N')";
		...
		$message = "<tr><td colspan='2' style='color:black;padding:20px;' align='center' valign='top'><h3>
					Thank you for signing up for the Newsletter(s)! 
					You will receive a welcome e-mail confirming your subscription. 
					Please allow 24-48 hours to receive your first newsletter. 
					</h3></td></tr>";
		$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=s&f=WimDhtml$src' width='0' height='0' border='0' />";
	}
} else {
	$attempt = false;
	$src = '';
	...
	$src = strtoupper($src);
	$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=d&f=WimDhtml$src' width='0' height='0' border='0' />";
}
...
<table width="610px" height="300px" align="center" border="0" cellpadding="0" cellspacing="0" style="font-size:11px;font-family: verdana;background-image:url('http://pics.workitmom.com/dhtml_images/3332_WIM.jpg');background-repeat:no-repeat;">
<tr valign="top">
...
<tr>
	<td colspan="2" align="center">
		<font size="1">
			<input type="checkbox" id="agree" name="agree" value="Y" <?php if ($agree == 'Y') { echo ' checked '; } ?>> I accept the <a href="http://workitmom.com/terms/?gclid=1" target="_blank">Terms & Conditions</a> 
			and <a href="http://workitmom.com/privacy/?gclid=1" target="_blank">Privacy Policy</a>.  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="http://workitmom.com/about/?gclid=1" target="_blank">About</a> | <a href="http://wim.popularliving.com/subctr/unsub/unsub_wim.php" target="_blank">Unsubscribe</a>
		</font>
	</td>
</tr>
<tr><td colspan="2">&nbsp;</td></tr>
