...
if ($_POST['submit'] == 'Sign Me Up!') {
	...
	if ($src == '' || $src == 'summer') {
		$redirect_to_url = 'http://www.awesomeopinions.com/dispatch2.asp?home=1771-28782W-L1';
	}
	...
	if ($error != '') {
		$message = "<tr><td colspan='3' style='color:red;' align='center' valign='top'>$error</td></tr>";
		$signup_success = false;
	} else {
		// process sign up request...
		$aJoinListId = array('410','411');
		$signup_success = true;
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"2960\",\"FFSiteSummerDhtml\",\"$src\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"2960\",\"FFSiteSummerDhtml\",\"$src\",'sub','N')";
		...
		if ($build_list_id != '') {
			...
			$send_to_arcamax = Arcamax($email,$build_list_id,'2960',$user_ip,'sub'); // sub or unsub
			...
			$insert_log = "INSERT IGNORE INTO arcamaxNewLog (dateTime,email,listid,subcampid,ipaddr,type,response)
						VALUES (NOW(),\"$email\",\"$build_list_id\",\"2960\",\"$user_ip\",\"sub\",\"$send_to_arcamax\")";
			$insert_log_result = mysql_query($insert_log);
			echo mysql_error();
		}
		
		$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=s&f=$src' width='0' height='0' border='0' />";
		
		echo "$pixel<script>
				function call_to_flow() {
					var t=setTimeout('redirect_to_flow()',1000);
				}
				function redirect_to_flow() {
					self.parent.location='$redirect_to_url&email=$email';
				}
				call_to_flow();
				</script>You are being redirected....please wait";
		exit;
	}
} else {
	...
	$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=d&f=$src' width='0' height='0' border='0' />";
}
...
<table border="0"  width="640px" align="center" cellpadding="0" cellspacing="0">
<?php echo $message; if ($signup_success == true) { exit; } ?>
<tr align="center">
	<td valign="top" align="left" colspan="3">
		<img src="http://pics.fitandfabliving.com/dhtml/FitandFab_header2.gif">
		<img src="http://pics.fitandfabliving.com/dhtml/step1headeremailcap_right.gif">
		<br>
	</td>
</tr>
<tr align="center">
	<td valign="top" colspan="3">
		<img src="http://pics.fitandfabliving.com/dhtml/616x300_Summer_eBook_Landing.jpg">
	</td>
</tr>
<tr align="center"><td colspan="3">&nbsp;</td></tr>
<tr align="center">
	<td align="right" style="font-family:Arial,Helvetica,sans-serif;color:rgb(102,102,102);font-size:20px;font-weight:bold;">Enter Your Email</td>
	<td><img src="http://pics.fitandfabliving.com/dhtml/blue_arrow_63x29.gif" border="0"></td>
	<td align="left"><input type="text" name="email" size="25" maxlength="100"></td>
</tr>
<tr align="center"><td colspan="3">&nbsp;</td></tr>
<tr align="center"><td colspan="2" align="right"><input type="submit" name="submit" value="Sign Me Up!"></td>
<td align="right"><a href="http://www.fitandfabliving.com/index.php/privacy-policy.html?gclid=1" target="_blank"><font size="2">Privacy Policy</font></a></td>
</tr>
</table>
</form>
