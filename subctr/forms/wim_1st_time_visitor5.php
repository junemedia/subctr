	...
	if ($error != '') {
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$subcampid\",\"WIMDHTML\",\"WIMFIRSTTIME5\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"$subcampid\",\"WIMDHTML\",\"WIMFIRSTTIME5\",'sub','N')";
		...
		$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=s&f=WIMFIRSTTIME5' width='0' height='0' border='0' />";
	}
} else {
	$attempt = false;
	$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=d&f=WIMFIRSTTIME5' width='0' height='0' border='0' />";
}
...
<div style="margin: 0 auto;font-size:11px;font-family: verdana;background-image:url('http://pics.workitmom.com/WIM-Dhtml-first-time-vistor5A.jpg');background-repeat:no-repeat;position: relative; width:610px;height:300px;">
	<div style="position: absolute;top: 195px; left: 425px;height:25px;width:150px;">
    E-mail Address: <input type="text" name="email" value="<?php echo $email; ?>" size="25" maxlength="100">
	</div>
	<input type="hidden" name="submit" value="Sign Me Up!">
	<div style="position: absolute;top: 240px; left: 425px;height:25px;width:150px;">
    <INPUT TYPE="image" SRC="http://pics.workitmom.com/WIM-Dhtml-first-time-vistorSubmitA.jpg" BORDER="0" ALT="SUBMIT!">
	</div>
</div>
