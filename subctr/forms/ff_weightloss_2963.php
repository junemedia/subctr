	...
	if ($error != '') {
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"2963\",\"FFDHTML\",\"$src\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"2963\",\"FFDHTML\",\"$src\",'sub','N')";
		...
		if ($build_list_id != '') {
			...
			$send_to_arcamax = Arcamax($email,$build_list_id,'2963',$user_ip,'sub'); // sub or unsub
			...
			$insert_log = "INSERT IGNORE INTO arcamaxNewLog (dateTime,email,listid,subcampid,ipaddr,type,response)
						VALUES (NOW(),\"$email\",\"$build_list_id\",\"2963\",\"$user_ip\",\"sub\",\"$send_to_arcamax\")";
			$insert_log_result = mysql_query($insert_log);
			echo mysql_error();
		}
		
		$message = "<meta http-equiv='Refresh' content='1;url=http://www.internetinput.com/dispatch2.asp?home=1765-29247C&email=$email' />
					<tr><td colspan='2' style='color:black;padding:20px;' align='center' valign='top'><h3>
					Thank you for signing up for the Diet Insider newsletters!  
...
<table width="660px" height="377px" align="center" border="0" cellpadding="0" cellspacing="0" style="font-size:11px;font-family: verdana;background-image:url('http://pics.fitandfabliving.com/FF_shrimp_dhtml-step1of3Google.jpg');background-repeat:no-repeat;">
<tr valign="top">
...
<tr>
	<td colspan="2" align="right">
		<font size="1">[<a href="http://www.fitandfabliving.com/index.php/privacy-policy.html?gclid=1" target="_blank">Privacy</a>]</font>
	</td>
</tr>
</table>
</form>
