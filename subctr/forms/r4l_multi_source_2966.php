	...
	if ($error != '') {
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"2966\",\"R4LDhtml\",\"$src\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"2966\",\"R4LDhtml\",\"$src\",'sub','N')";
		...
		if ($build_list_id != '') {
			...
			$send_to_arcamax = Arcamax($email,$build_list_id,'2966',$user_ip,'sub'); // sub or unsub
			...
			$insert_log = "INSERT IGNORE INTO arcamaxNewLog (dateTime,email,listid,subcampid,ipaddr,type,response)
						VALUES (NOW(),\"$email\",\"$build_list_id\",\"2966\",\"$user_ip\",\"sub\",\"$send_to_arcamax\")";
			$insert_log_result = mysql_query($insert_log);
			echo mysql_error();
		}
		
		$message = "<meta http-equiv='Refresh' content='0;url=http://www.supersurveysdaily.com/dispatch2.asp?home=1786-29246M&email=$email' />
					<tr><td colspan='2' style='color:black;padding:20px;' align='center' valign='top'><h3>
					Thank you for signing up for the Crockpot Creations newsletter!  
...
<table width="660px" height="377px" align="center" border="0" cellpadding="0" cellspacing="0" style="font-size:11px;font-family: verdana;background-image:url('http://pics.recipe4living.com/r4l_search_LP-crockpot_step1of3_2966.jpg');background-repeat:no-repeat;">
<tr valign="top">
