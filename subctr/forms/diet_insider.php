	...
	if ($error != '') {
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"2925\",\"DietInsider\",\"DietInsider\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"2925\",\"DietInsider\",\"DietInsider\",'sub','N')";
		...
		if ($build_list_id != '') {
			...
			$send_to_arcamax = Arcamax($email,$build_list_id,'2925',$user_ip,'sub'); // sub or unsub
			...
			$insert_log = "INSERT IGNORE INTO arcamaxNewLog (dateTime,email,listid,subcampid,ipaddr,type,response)
						VALUES (NOW(),\"$email\",\"$build_list_id\",\"2925\",\"$user_ip\",\"sub\",\"$send_to_arcamax\")";
		...
		$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=s&f=DietInsider' width='0' height='0' border='0' />";
	}
} else {
	$attempt = false;
	$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=d&f=DietInsider' width='0' height='0' border='0' />";
}
...
function check_fields() {
	...
	if (document.form1.email.value == '') {
		...
		if (!document.getElementById(1).checked) {
			str += "You have not checked newsletter. Please select newsletter to proceed.";
			document.form1.email.style.backgroundColor="yellow";
		}
	}
...
<table border="0" height="300px" width="500px" cellpadding="0" cellspacing="0" bgcolor="LightBlue" align='center'>
<tr>
			...
			<tr>
				<td colspan="2">
					<table width="100%" align="center" style="font-size:11px;font-family: verdana;" border="0" cellpadding="2" cellspacing="0">
						<tr>
							<td>
								<input type="checkbox" name="aJoinListId[]" value="448" id="1" checked> <b>Diet Insider</b> (2 issues/week)
							</td>
						</tr>
