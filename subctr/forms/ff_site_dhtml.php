	...
	if ($error != '') {
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"2873\",\"FFSiteDhtml\",\"FFSiteDhtml\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"2873\",\"FFSiteDhtml\",\"FFSiteDhtml\",'sub','N')";
		...
		if ($build_list_id != '') {
			...
			$send_to_arcamax = Arcamax($email,$build_list_id,'2873',$user_ip,'sub'); // sub or unsub
			...
			$insert_log = "INSERT IGNORE INTO arcamaxNewLog (dateTime,email,listid,subcampid,ipaddr,type,response)
						VALUES (NOW(),\"$email\",\"$build_list_id\",\"2873\",\"$user_ip\",\"sub\",\"$send_to_arcamax\")";
		...
		$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=s&f=FFSiteDhtml' width='0' height='0' border='0' />";
	}
} else {
	$attempt = false;
	$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=d&f=FFSiteDhtml' width='0' height='0' border='0' />";
}

$bg_img_array = array('http://pics.fitandfabliving.com/FitandFabNewsletter_FULL_BG_NEW.jpg',
					'http://pics.fitandfabliving.com/FitandFabNewsletter_FULL_BG_NEW.jpg',
					'http://pics.fitandfabliving.com/FitandFabNewsletter_FULL_BG_NEW.jpg');
...
<tr align="left" style="padding-left:0px;">
	...
	<td width="340px" valign="top" style="padding-top:88px;padding-right:20px;">
		<table height="300px" width="90%" border="0" style="background-color:white;border:0px;font-size:11px;font-family: verdana;" align="right" bgcolor="White">
			...
			<div id="t1" class="tip">Look and feel your best with tips on diet, fashion trends, beauty, fitness and more!</div>
			<div id="t3" class="tip">From makeup advice to hair style ideas and more, get the latest beauty trends in your inbox!</div>
