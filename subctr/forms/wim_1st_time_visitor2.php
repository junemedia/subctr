	...
	if ($error != '') {
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$subcampid\",\"WIMDHTML\",\"WIMFIRSTTIME2\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"$subcampid\",\"WIMDHTML\",\"WIMFIRSTTIME2\",'sub','N')";
		...
		$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=s&f=WIMFIRSTTIME2' width='0' height='0' border='0' />";
	}
} else {
	$attempt = false;
	$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=d&f=WIMFIRSTTIME2' width='0' height='0' border='0' />";
}
...
<table width="610px" height="300px" align="center" border="0" cellpadding="0" cellspacing="0" style="font-size:11px;font-family: verdana;background-image:url('http://pics.workitmom.com/WIM-Dhtml-first-time-vistor2.jpg');background-repeat:no-repeat;">
<tr valign="top">
