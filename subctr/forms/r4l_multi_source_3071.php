...
if ($_POST['submit'] == 'Submit for FREE Access') {
	...
	if ($error != '') {
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"3071\",\"R4LFacebook3071\",\"$src\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"3071\",\"R4LFacebook3071\",\"$src\",'sub','N')";
		...
		if ($build_list_id != '') {
			...
			$send_to_arcamax = Arcamax($email,$build_list_id,'3071',$user_ip,'sub'); // sub or unsub
			...
			$insert_log = "INSERT IGNORE INTO arcamaxNewLog (dateTime,email,listid,subcampid,ipaddr,type,response)
						VALUES (NOW(),\"$email\",\"$build_list_id\",\"3071\",\"$user_ip\",\"sub\",\"$send_to_arcamax\")";
		...
		$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=s&f=R4LFacebook3071$src' width='0' height='0' border='0' />";
	}
} else {
	$attempt = false;
	$src = '';
	...
	$src = strtoupper($src);
	$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=d&f=R4LFacebook3071$src' width='0' height='0' border='0' />";
}
...
<table width="600px" height="300px" align="center" border="0" cellpadding="0" cellspacing="0" style="font-size:11px;font-family: verdana;background-image:url('http://pics.recipe4living.com/3071_r4l_facebook_dhtml.jpg');background-repeat:no-repeat;">
<tr valign="top">
