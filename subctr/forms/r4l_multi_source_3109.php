...
if ($_POST['submit'] == 'Sign Me Up!') {
	...
	if ($error != '') {
		...
		foreach ($aJoinListId as $listid) {
			...
			$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"3109\",\"FacebookQE3109\",\"$src\")";
			...
			$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"3109\",\"FacebookQE3109\",\"$src\",'sub','N')";
		...
		if ($build_list_id != '') {
			...
			$send_to_arcamax = Arcamax($email,$build_list_id,'3109',$user_ip,'sub'); // sub or unsub
			...
			$insert_log = "INSERT IGNORE INTO arcamaxNewLog (dateTime,email,listid,subcampid,ipaddr,type,response)
						VALUES (NOW(),\"$email\",\"$build_list_id\",\"3109\",\"$user_ip\",\"sub\",\"$send_to_arcamax\")";
		...
		$message = "<tr><td colspan='2' style='color:black;padding:20px;' align='center' valign='top'><h3>
					Thank you for signing up for the Quick & Easy newsletter!  
					You will receive a welcome e-mail confirming your subscription. 
					Please allow 24-48 hours to receive your first newsletter.
					</h3></td></tr>";
		$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=s&f=FacebookQE3109$src' width='0' height='0' border='0' />";
	}
} else {
	$attempt = false;
	$src = '';
	...
	$src = strtoupper($src);
	$pixel = "<img src='http://".trim($_SERVER['SERVER_NAME'])."/subctr/forms/stats.php?a=d&f=FacebookQE3109$src' width='0' height='0' border='0' />";
}
...
<table width="600px" height="300px" align="center" border="0" cellpadding="0" cellspacing="0" style="font-size:11px;font-family: verdana;background-image:url('http://pics.recipe4living.com/3109_QE_email.jpg');background-repeat:no-repeat;">
<tr valign="top">
...
<tr>
	<td colspan="2" align="center">
		<font size="2">
			<input type="checkbox" id="agree" name="agree" value="Y" <?php if ($agree == 'Y') { echo ' checked '; } ?>> I accept the <a href="http://www.recipe4living.com/terms" target="_blank">Terms & Conditions</a> 
			and <a href="http://www.recipe4living.com/privacy" target="_blank">Privacy Policy</a>.
		</font>
	</td>
</tr>
<tr><td colspan="2">&nbsp;</td></tr>
