...
if (isset($_POST['submit']) && $_POST['submit'] == 'Submit') {
  ...
  if ($_REQUEST['action_stay'] == 'Y') {
    header("Location:/subctr/unsub/br/stay.php?listid=$listid&jobid=$jobid&email=$email");
    exit;
  }
  if ($_REQUEST['action_unsub'] == 'Y') {
    header("Location:/subctr/unsub/br/unsub.php?listid=$listid&jobid=$jobid&email=$email");
    exit;
  }
} else {
  $email = trim($_REQUEST['email']);
...
switch ($listid) {
  ...
  case '505':
    ...
    break;
  default:
    echo "Invalid unsubscribe link: <a href='http://br.popularliving.com/subctr/index.php'>Click here to go to Subscription Center to unsubscribe</a>";
    exit;
}
