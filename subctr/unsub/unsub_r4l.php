...
$title_393 = 'Daily Recipes';
$title_396 = 'Recipe4Living Special Offers';
$title_395 = 'Budget Cooking';
$title_394 = 'Quick & Easy Recipes';
$title_511 = 'Crockpot Creations';
$title_539 = "Casserole Cookin'";
$title_554 = 'Copycat Classics';
$title_574 = 'Diabetic-Friendly Dishes';
...
if ($_POST['submit'] == 'Unsubscribe') {
	...
	if ($message == '') {
		foreach ($aListId as $listid) {
			...
			if (mysql_num_rows($check_query_result) > 0) {
				...
				$insert_query = "INSERT INTO joinEmailUnsub (dateTime,email,ipaddr,listid,subcampid,source,subsource,errorCode)
							VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"2639\",\"R4LUnsubLink\",\"$subsource\",\"per request\")";
				...
				$send_to_arcamax = Arcamax($email,$listid,'2639',$user_ip,'unsub'); // sub or unsub
				...
				$insert_log = "INSERT INTO arcamaxNewLog (dateTime,email,listid,subcampid,ipaddr,type,response)
							VALUES (NOW(),\"$email\",\"$listid\",\"2639\",\"$user_ip\",\"unsub\",\"$send_to_arcamax\")";
				$insert_log_result = mysql_query($insert_log);
				echo mysql_error();
			} else {
				// call to function to send unsub to Arcamax
				// unsub user even this user is not in Nibbles database.  this is just to make sure user is unsub even if nibbles/arcamax db are not in sync
				$send_to_arcamax = Arcamax($email,$listid,'2639',$user_ip,'unsub'); // sub or unsub
			}
		}
		...
		echo "<table width='500px' align='center'><tr><td><font style='font-family:arial;font-style:normal;font-size:12px;font-weight:normal;text-decoration:none;'>
				...
				re-subscribe <a href='http://www.recipe4living.com/index/subctr'>here</a>.
				</font></td></tr></table>";
...
function check_fields() {
	...
	if (!(document.getElementById('393').checked || document.getElementById('394').checked || document.getElementById('395').checked || document.getElementById('396').checked || document.getElementById('511').checked || document.getElementById('539').checked || document.getElementById('554').checked || document.getElementById('574').checked)) {
...
function CheckAll () {
	document.getElementById('393').checked = true;
	document.getElementById('396').checked = true;
	document.getElementById('395').checked = true;
	document.getElementById('394').checked = true;
	document.getElementById('511').checked = true;
	document.getElementById('539').checked = true;
	document.getElementById('554').checked = true;
	document.getElementById('574').checked = true;
}
function UnCheckAll () {
	document.getElementById('393').checked = false;
	document.getElementById('396').checked = false;
	document.getElementById('395').checked = false;
	document.getElementById('394').checked = false;
	document.getElementById('511').checked = false;
	document.getElementById('539').checked = false;
	document.getElementById('554').checked = false;
	document.getElementById('574').checked = false;
}
</script>
...
<form name="form1" method="POST" action="<?php echo $_SERVER['PHP_SELF']?>" onsubmit="return check_fields();">
	...
	<table width="500" align="center" style="font-style: normal;font-size: 12px;font-weight: normal;text-decoration: none;">
		<tr>
			...
			<font color="#0F22B0" size="4" style="font-family: arial;">Unsubscribe From Recipe4Living Newsletters</font>
			</td>
		</tr>
	</table>
	<center><h2><?php echo $message; ?></h2></center>
	
	<table width="500" align="center" id="unsub_form" border="0" style="font-style: normal;font-size: 12px;font-weight: normal;text-decoration: none;">
		<tr>
		...
		<tr>
			<td><input type="checkbox" id='393' name="aListId[]" value="393" <?php if ($id == '393') { echo ' checked '; } ?>> <b><?php echo $title_393; ?></b></td>
		</tr>
		
		<tr>
			<td><input type="checkbox" id='396' name="aListId[]" value="396" <?php if ($id == '396') { echo ' checked '; } ?>> <b><?php echo $title_396; ?></b></td>
		</tr>
		
		<tr>
			<td><input type="checkbox" id='395' name="aListId[]" value="395" <?php if ($id == '395') { echo ' checked '; } ?>> <b><?php echo $title_395; ?></b></td>
		</tr>
		
		<tr>
			<td><input type="checkbox" id='394' name="aListId[]" value="394" <?php if ($id == '394') { echo ' checked '; } ?>> <b><?php echo $title_394; ?></b></td>
		</tr>
		
		<tr>
			<td><input type="checkbox" id='511' name="aListId[]" value="511" <?php if ($id == '511') { echo ' checked '; } ?>> <b><?php echo $title_511; ?></b></td>
		</tr>
		
		<tr>
			<td><input type="checkbox" id='539' name="aListId[]" value="539" <?php if ($id == '539') { echo ' checked '; } ?>> <b><?php echo $title_539; ?></b></td>
		</tr>
		
		<tr>
			<td><input type="checkbox" id='554' name="aListId[]" value="554" <?php if ($id == '554') { echo ' checked '; } ?>> <b><?php echo $title_554; ?></b></td>
		</tr>
		
		<tr>
			<td><input type="checkbox" id='574' name="aListId[]" value="574" <?php if ($id == '574') { echo ' checked '; } ?>> <b><?php echo $title_574; ?></b></td>
		</tr>
		<tr>
			<td colspan="2">&nbsp;</td>
		</tr>
		<tr>
			<td colspan="2"><a href="#" onclick="CheckAll();"><span style="color:#0F22B0;">Check All</span></a>
			&nbsp;&nbsp;&nbsp;&nbsp;
			<a href="#" onclick="UnCheckAll();"><span style="color:#0F22B0;">Uncheck All</span></a>
			</td>
		</tr>
		<tr>
			<td>&nbsp;</td>
