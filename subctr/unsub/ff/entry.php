...
if ($_POST['submit'] == 'Submit') {
	...
	if ($_REQUEST['action_unsub'] == 'Y') {
		header("Location:http://fitfab.popularliving.com/subctr/unsub/ff/unsub.php?listid=$listid&jobid=$jobid&email=$email");
		exit;
	}
} else {
	//$email = trim($_REQUEST['email']);
	...
	if (!ctype_digit($jobid)) { $jobid = 0; }
	$listid = trim($_REQUEST['listid']);
	$pixel = "<img src='http://www.fitandfabliving.com/unsub/".$listid."_Form.html' width='0' height='0' border='0'>";
}
