...
if (isset($_POST['submit']) && $_POST['submit'] == 'Submit') {
  ...
  if (isset($_REQUEST['action_stay']) && $_REQUEST['action_stay'] == 'Y') {
    header("Location:/subctr/unsub/ff/stay.php?listid=$listid&jobid=$jobid&email=$email");
    exit;
  }
    ...
    if ($message == '') {
      ...
      if (mysql_num_rows($check_query_result) > 0) {
        ...
        $campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
                VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"2744\",\"FFUnsubLink\",\"\",'unsub','Y')";
        ...
        $insert_query = "INSERT IGNORE INTO joinEmailUnsub (dateTime,email,ipaddr,listid,subcampid,source,subsource,errorCode)
              VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"2744\",\"FFUnsubLink\",\"$jobid\",\"per request\")";
      ...
      header("Location:/subctr/unsub/ff/unsub_conf.php?listid=$listid&jobid=$jobid&email=$email");
      exit;
    }
  }
} else {
  ...
  $pixel = "<img src='http://www.fitandfabliving.com/unsub/".$listid."_Unsub.html' width='0' height='0' border='0'>";
}

switch ($listid) {
  case '411':
    $newsletter_name = 'Daily Insider';
    break;
  case '410':
    $newsletter_name = 'Fit&Fab Living Special Offers';
    break;
  case '448':
    $newsletter_name = 'Diet Insider';
    break;
  default:
...
<form name="form1" method="POST" action="<?php echo $_SERVER['PHP_SELF']?>" onsubmit="return check_fields();">
  ...
  <table width="450" align="center" style="font-style: normal;font-size: 12px;font-weight: normal;text-decoration: none;">
    <tr>
      <td>
      <font color="#F07AB0" size="4" style="font-family: arial;"><i><b>Wait - don't go!</b></i>  Are you sure you want to be unsubscribed from <i><b><?php echo $newsletter_name; ?></b></i>?</font>
      </td>
    </tr>
    <tr><td>&nbsp;</td></tr>
    <tr>
    ...
    <tr>
      <td>
        <input type="radio" name="a1" id="a1_1" value="I get too many emails from you." <?php if (isset($a1) && $a1 == 'I get too many emails from you.') { echo ' checked '; } ?> onclick="javascript:document.getElementById('other').disabled=true;">I get too many emails from you.
        <br><br>
        <input type="radio" name="a1" id="a1_2" value="I am not interested in the content of these emails." <?php if (isset($a1) && $a1 == 'I am not interested in the content of these emails.') { echo ' checked '; } ?> onclick="javascript:document.getElementById('other').disabled=true;">I am not interested in the content of these emails.
        <br><br>
        <input type="radio" name="a1" id="a1_3" value="I dont remember signing up for these emails." <?php if (isset($a1) && $a1 == "I dont remember signing up for these emails.") { echo ' checked '; } ?> onclick="javascript:document.getElementById('other').disabled=true;">I don't remember signing up for these emails.
        <br><br>
        <input type="radio" name="a1" id="a1_4" value="OTHER" <?php if (isset($a1) && $a1 == 'OTHER') { echo ' checked '; } ?> onclick="javascript:document.getElementById('other').disabled=false;">Other:
        <input type="text" maxlength="100" size="40" name="other" id="other" value="<?php if (isset($other)) { echo $other; } ?>">
      </td>
    </tr>
    ...
    <tr><td><textarea name="comments" id="comments" cols="37" rows="5" maxlength="200"><?php if (isset($comments)) { echo $comments; } ?></textarea></td></tr>
