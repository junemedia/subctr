...
if (isset($_POST['submit']) && $_POST['submit'] == 'Submit') {
    ...
    if ($message == '') {
      ...
      if (mysql_num_rows($check_query_result) > 0) {
        ...
        $campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
                VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"$new_listid\",\"3078\",\"WIMUnsubLink\",\"\",'unsub','Y')";
        ...
        $insert_query = "INSERT IGNORE INTO joinEmailUnsub (dateTime,email,ipaddr,listid,subcampid,source,subsource,errorCode)
              VALUES (NOW(),\"$email\",\"$user_ip\",\"$listid\",\"3078\",\"WIMUnsubLink\",\"$jobid\",\"per request\")";
      ...
      $send_to_arcamax = Arcamax($email,$listid,'3078',$user_ip,'unsub'); // sub or unsub
      ...
      $insert_log = "INSERT IGNORE INTO arcamaxNewLog (dateTime,email,listid,subcampid,ipaddr,type,response)
            VALUES (NOW(),\"$email\",\"$listid\",\"3078\",\"$user_ip\",\"unsub\",\"$send_to_arcamax\")";
      $insert_log_result = mysql_query($insert_log);
      echo mysql_error();

      header("Location:/subctr/unsub/wim/unsub_conf.php?listid=$listid&jobid=$jobid&email=$email");
      exit;
    }
  }
} else {
  ...
  $pixel = "<img src='http://www.workitmom.com/unsub/".$listid."_Unsub.html' width='0' height='0' border='0'>";
}
