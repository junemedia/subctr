...
/*
Method: GET or POST
Posting URL: http://fitfab.popularliving.com/wim_api_giveaway.php
Required Fields: email=&sublists=&subcampid=&ipaddr=&keycode=kfdj49358gkj359gjk55
Example: http://fitfab.popularliving.com/wim_api_giveaway.php?email=samirp@silvercarrot.com&sublists=395,396&subcampid=2679&ipaddr=216.180.167.121&keycode=kfdj49358gkj359gjk55
*/
...
foreach($parts as $list_parts) {
	$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source)
				VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$subcampid\",\"WIMGiveaway\")";
	...
	$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
					VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$new_listid\",\"$subcampid\",\"WIMGiveaway\",\"\",'sub','N')";
...
$insert_api_lead = "INSERT INTO api (dateTimeAdded,email,listid,subcampid,ipaddr,subsource) 
		VALUES (NOW(),\"$email\",\"$sublists\",\"$subcampid\",\"$ipaddr\",'WIMGiveaway')";
