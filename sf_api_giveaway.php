...
/*
Method: GET or POST
Posting URL: http://sf.popularliving.com/sf_api_giveaway.php
Required Fields: email=&sublists=&subcampid=&ipaddr=&keycode=ggjig592fkg785kscm8473
Example: http://sf.popularliving.com/sf_api_giveaway.php?email=samirp@silvercarrot.com&sublists=395,396&subcampid=2679&ipaddr=216.180.167.121&keycode=ggjig592fkg785kscm8473
*/
...
if ($keycode != 'ggjig592fkg785kscm8473') { echo 'Error: keycode';exit; }
...
foreach($parts as $list_parts) {
	$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source)
				VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$subcampid\",\"SFGiveaway\")";
	$insert_query_result = mysql_query($insert_query);
	echo mysql_error();

	$insert_query = "INSERT IGNORE INTO joinEmailActive (dateTime,email,ipaddr,listid,subcampid,source)
					VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$subcampid\",\"SFGiveaway\")";
	$insert_query_result = mysql_query($insert_query);
	echo mysql_error();
	...
	$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
					VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$new_listid\",\"$subcampid\",\"SFGiveaway\",\"\",'sub','N')";
...
$insert_api_lead = "INSERT INTO api (dateTimeAdded,email,listid,subcampid,ipaddr,subsource) 
		VALUES (NOW(),\"$email\",\"$sublists\",\"$subcampid\",\"$ipaddr\",'SFGiveaway')";
...
$update = "UPDATE userData SET fname=\"$fname\" WHERE email=\"$email\" LIMIT 1";
$update_log_result = mysql_query($update);
echo mysql_error();
