...
/*
Method: GET or POST
Posting URL: http://fitfab.popularliving.com/m_fitfab_api.php
Required Fields: email=&sublists=&subcampid=&ipaddr=&keycode=nu6u7ach
Example: http://fitfab.popularliving.com/m_fitfab_api.php?email=samirp@silvercarrot.com&sublists=395,396&subcampid=2679&ipaddr=216.180.167.121&keycode=nu6u7ach
*/
...
if ($keycode != 'nu6u7ach') { echo 'Error: keycode';exit; }
...
foreach($parts as $list_parts) {
	$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
				VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$subcampid\",\"MobileFitFab\",\"$subsource\")";
	...
	$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
					VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$new_listid\",\"$subcampid\",\"MobileFitFab\",\"$subsource\",'sub','N')";
...
$insert_api_lead = "INSERT INTO api (dateTimeAdded,email,listid,subcampid,ipaddr,subsource) 
		VALUES (NOW(),\"$email\",\"$sublists\",\"$subcampid\",\"$ipaddr\",'MobileFitFab')";
