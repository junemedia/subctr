...
/*
Method: GET or POST
Posting URL: http://fitfab.popularliving.com/ff_api.php
Required Fields: email=&sublists=&subcampid=&ipaddr=&keycode=ggjig592fkg785kscm8473
Example: http://fitfab.popularliving.com/ff_api.php?email=samirp@silvercarrot.com&sublists=395,396&subcampid=2679&ipaddr=216.180.167.121&keycode=ggjig592fkg785kscm8473&source=
*/
...
if ($keycode != 'ggjig592fkg785kscm8473') { echo 'Error: keycode'.$check_response;exit; }
...
foreach($parts as $list_parts) {
	$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source)
				VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$subcampid\",\"$source\")";
	...
	$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
					VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$new_listid\",\"$subcampid\",\"$source\",\"\",'sub','N')";
...
$insert_api_lead = "INSERT INTO api (dateTimeAdded,email,listid,subcampid,ipaddr,subsource) 
		VALUES (NOW(),\"$email\",\"$sublists\",\"$subcampid\",\"$ipaddr\",'$source')";
