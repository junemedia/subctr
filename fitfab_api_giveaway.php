...
/*
Method: GET or POST
Posting URL: http://fitfab.popularliving.com/fitfab_api_giveaway.php
...
foreach($parts as $list_parts) {
	$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
				VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$subcampid\",\"FitFabGiveaway\",\"$subsource\")";
	...
	$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
					VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$new_listid\",\"$subcampid\",\"FitFabGiveaway\",\"$subsource\",'sub','N')";
...
$insert_api_lead = "INSERT INTO api (dateTimeAdded,email,listid,subcampid,ipaddr,subsource) 
		VALUES (NOW(),\"$email\",\"$sublists\",\"$subcampid\",\"$ipaddr\",'FitFabGiveaway')";
...
$update = "UPDATE userData SET fname=\"$fname\", lname=\"$lname\", gender=\"$gender\", age_group=\"$age_group\" WHERE email=\"$email\" LIMIT 1";
