...
Posting URL: http://r4l.popularliving.com/m_r4l_api.php
Required Fields: email=&sublists=&subcampid=&ipaddr=&keycode=g94khd564gh
Example: http://r4l.popularliving.com/m_r4l_api.php?email=samirp@silvercarrot.com&sublists=393,396&subcampid=3213&ipaddr=216.180.167.121&keycode=g94khd564gh
*/
...
if ($keycode != 'g94khd564gh') { echo 'Error: keycode';exit; }
...
foreach($parts as $list_parts) {
	$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
				VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$subcampid\",\"MobileR4L\",\"$subsource\")";
	...
	$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
					VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$new_listid\",\"$subcampid\",\"MobileR4L\",\"$subsource\",'sub','N')";
...
$insert_api_lead = "INSERT INTO api (dateTimeAdded,email,listid,subcampid,ipaddr,subsource) 
		VALUES (NOW(),\"$email\",\"$sublists\",\"$subcampid\",\"$ipaddr\",'MobileR4L')";
