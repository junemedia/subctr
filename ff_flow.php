...
Posting URL: http://fitfab.popularliving.com/ff_flow.php
Required Fields: email=&ipaddr=&keycode=gjhk5487gdfhkjg9438&sublists=&subcampid=&subsource=
Example: https://fitfab.popularliving.com/ff_flow.php?email=samirp@silvercarrot.com&ipaddr=216.180.167.121&keycode=gjhk5487gdfhkjg9438&sublists=393,396&subcampid=1234&subsource=LINKID
*/
...
if ($keycode != 'gjhk5487gdfhkjg9438') { echo 'Error: keycode'.$check_response;exit; }
...
foreach($parts as $list_parts) {
	$insert_query = "INSERT IGNORE INTO joinEmailSub (dateTime,email,ipaddr,listid,subcampid,source,subsource)
				VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$subcampid\",\"FFFlowDhtml\",\"$subsource\")";
	...
	$campaigner = "INSERT IGNORE INTO campaigner (dateTime,email,ipaddr,oldListId,newListId,subcampid,source,subsource,type,isProcessed)
					VALUES (NOW(),\"$email\",\"$ipaddr\",\"$list_parts\",\"$new_listid\",\"$subcampid\",\"FFFlowDhtml\",\"$subsource\",'sub','N')";
...
$insert_api_lead = "INSERT IGNORE INTO api (dateTimeAdded,email,listid,subcampid,ipaddr,subsource) 
		VALUES (NOW(),\"$email\",\"$sublists\",\"$subcampid\",\"$ipaddr\",'FFFlowDhtml')";
